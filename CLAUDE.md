...
- **Publish ordinary workspace JSON through `atomic_state.write_json`.** For non-secret compact state records, use its unique per-invocation sibling temp, cleanup, and atomic replace rather than a shared `.json.tmp` recipe. Keep stricter permission/durability writers for credentials and other private state.
...
- **Serialize task-last records through `local_task_protocol.serialize_task`.** Migrate producers one at a time with byte-identical golden tests. Trusted metadata stays single-line and before `task:`; untrusted multi-line content belongs only after that delimiter. Do not hand-assemble a second task-file grammar.
- **Adapter handlers orchestrate; pure construction lives beside them.** Keep receive handlers focused on provider I/O and sequencing. Move deterministic envelope/payload construction into dependency-light adjacent modules, inject timestamps and resolved values, and pin the exact bytes before extraction. Do not move or rewrite tuned prompts while splitting a handler.
