...
- **The Sutando config schema owns top-level config keys.** Edit `docs/sutando-config.schema.json`, then run `python3 scripts/gen-config-key-contract.py`; do not hand-edit the generated Python or TypeScript key catalogs. Commit the schema and both generated loader updates together.
